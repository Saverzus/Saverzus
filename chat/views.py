	...
	if user1.is_authenticated:
		if request.method == "POST":
			...
			try:
				...
				payload['response'] = "Удачно подключились к чату"
				payload['chatroom_id'] = chat.id
			except Account.DoesNotExist:
				payload['response'] = "Не удалось начать чат с этим пользователем."
	else:
		payload['response'] = "Вы не можете начать чат, если вы не авторизованы."
